...
3.  **メジャーバージョンタグの自動更新**: リリース公開時に、メジャーバージョンタグ（例: `v0`）を最新へ更新。
...
4.  **Version** 入力欄は、**基本的には空欄のままでOKです**。
    *   空欄の場合、現在作成されている「Draft Release」のタイトル（例: `v0.0.5`）からバージョンを自動取得します。
    *   もし手動で指定したい場合のみ、バージョンを入力してください（例: `0.0.5`）。
    *   **【注意】メジャーバージョンアップ（例: v0 -> v1）の場合**:
        *   READMEの更新は自動で行われますが、念のため生成されたPRの内容を確認してください。
...
1.  リポジトリの **[Releases](https://github.com/asabon/android-build-log-analyzer/releases)** ページを開きます。
2.  一覧の一番上に自動生成された **Update-vX.X.X (Draft)** というドラフトがあるので、右側の鉛筆アイコン（Edit）をクリックします。
3.  **Tag version** 欄に、先ほど Bump Version で指定したバージョン番号を入力します（例: `v0.0.5`）。
...
*   指定したバージョン（例: `v0.0.5`）のタグが作成されます。
*   **Update Major Tag** ワークフローが動き、メジャーバージョンタグ（例: `v0`）が強制的にこの新しいリリースを指すように更新されます。
